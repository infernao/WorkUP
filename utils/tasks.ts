  ...
  board: {
    getProjectTasks: async (projectId: string) => {
      ...
      return data.map((task) => ({
        ...
        assignees: task.task_assigness?.map((a) => a.users) || [],
        task_labels: null,
        task_assigness: null,
      })) as any[];
    },
  ...
  details: {
    get: async (taskId: string) => {
      ...
      return {
        ...
        assignees: data.task_assigness?.map((a: any) => a.users) || [],
        task_labels: null,
        task_assigness: null,
      } as ITaskWithOptions;
    },
    ...
    update: async (taskId: string, updates: Partial<ITask>) => {
      ...
      // Handle task_assigness junction table (existing code)
      if ('assignees' in updates) {
        ...
        await supabase.from('task_assigness').delete().eq('task_id', taskId);

        // If there are new assignees to add
        if (assigneeIds.length > 0) {
          // Insert new task-assignee relationships
          await supabase.from('task_assigness').insert(
